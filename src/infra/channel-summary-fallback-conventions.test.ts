  ...
  it("applies configured/detail rules for supported extension channels", () => {
    expect(
      resolveFallbackConventionConfigured("zulip", {
        ...
        botApiKey: "zulip-api-key", // pragma: allowlist secret
    ...
    expect(
      resolveFallbackConventionConfigured("twitch", {
        ...
        accessToken: "oauth:test123", // pragma: allowlist secret
    ...
    ).toEqual(["user:openclawbot", "channel:lionrootstudio", "token:config", "client:config"]);

    expect(
      resolveFallbackConventionConfigured("matrix", {
        homeserver: "https://matrix.example.org",
        userId: "@bot:example.org",
        accessToken: "tok-matrix", // pragma: allowlist secret
      }),
    ).toBe(true);
    expect(
    ...
    ).toEqual(["user:@bot:example.org", "homeserver:https://matrix.example.org", "token:config"]);

    expect(
      resolveFallbackConventionConfigured("msteams", {
        appId: "teams-app-id",
        appPassword: "teams-password", // pragma: allowlist secret
        tenantId: "tenant-123",
      }),
    ).toBe(true);
    expect(
    ...
    expect(
      resolveFallbackConventionConfigured("synology-chat", {
        token: "synology-token", // pragma: allowlist secret
