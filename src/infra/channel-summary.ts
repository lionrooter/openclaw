...
function readFallbackExtensionMeta(channelId: string): { label?: string; order?: number } | null {
  ...
  try {
    ...
    const meta = {
      label:
        typeof channelMeta?.label === "string" ? channelMeta.label.trim() || undefined : undefined,
...
function listFallbackChannelIds(cfg: OpenClawConfig): string[] {
  ...
  const remaining = configuredIds
    ...
    .toSorted(compareFallbackExtensionIds);
...
function resolveFallbackEnabled(
  account: unknown,
  channelConfig: Record<string, unknown> | null,
): boolean {
  const accountRecord = asRecord(account);
...
function selectFallbackAccount(params: {
  ...
  const sourceAccount =
    sourceInspected ?? resolveFallbackAccountConfig(sourceChannelConfig, params.accountId);
  ...
  const useSourceUnavailableAccount = Boolean(
    ...
    (!hasResolvedCredentialValue(resolvedAccount) || (sourceConfigured && !resolvedConfigured)),
  );
  ...
  const effectiveChannelConfig = useSourceUnavailableAccount
    ? sourceChannelConfig
...
async function buildFallbackChannelSummary(params: {
  ...
  for (const channelId of listFallbackChannelIds(params.cfg)) {
    ...
    const statusColor =
      status === "configured" ? theme.success : status === "disabled" ? theme.muted : theme.error;
